...
manifest_path = ROOT/'.maintenance/namespace-transfer.json'
if not manifest_path.exists():
    print('No pending namespace/secondary-document source transfer.'); raise SystemExit(0)
manifest = json.loads(manifest_path.read_text())
assert manifest['Schema'] == 1 and 1 <= manifest['Parts'] <= 32
parts = [ROOT/f'.maintenance/namespace.part{i:03d}' for i in range(manifest['Parts'])]
