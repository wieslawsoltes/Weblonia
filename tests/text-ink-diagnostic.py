"""Compare browser shaping, tiled alpha, and native upload without relaxing pixel gates."""
import json, os, re
...
with sync_playwright() as p:
    ...
    with p.chromium.launch(**launch) as b:
        ...
        report=page.evaluate(r'''async()=>{
            const A=await import('@wieslawsoltes/avalonia');
            const platform=await A.SkiaPlatform.Initialize({assetBaseUrl:new URL('vendor/skiasharpweb/dist/vendor/',document.baseURI).href,TextTileSize:128});
            ...
            for(const mode of ['direct-f32','translated','line-alpha','tiled-alpha']){
                ...
                for(const line of layout.TextLines){
                    ...
                    if(mode==='direct-f32'){ctx.setTransform(scale,0,0,scale,Math.fround(.31),Math.fround(.63));A.ConfigureCanvasText(ctx,layout.Typeface,24,layout);ctx.fillStyle='black';ctx.fillText(line.Text,bx,by);continue;}
            ...
            layout.Dispose();platform.Dispose();return info;
        }''')
