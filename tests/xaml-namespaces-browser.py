"""Namespace mapping/ignorable content in ordinary HTTP runtime and direct AOT.
...
OUT = ROOT/'artifacts/xaml-namespaces'; OUT.mkdir(parents=True, exist_ok=True)
subprocess.run(['node', '--import', './scripts/register-loader.mjs', 'scripts/compile-namespace-fixture.mjs'], cwd=ROOT, check=True)
report = {'Version': json.loads((ROOT/'package.json').read_text())['version'], 'SourceFingerprint': source_fingerprint(ROOT),
    ...
    'NamespaceMappings': True, 'PixelChannelTolerance': 2}
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/tests/namespace-entry.html':
            ...
            boot = {'ThreadingMode': mode, 'Backend': 'canvas',
                'ApplicationModule': f'http://127.0.0.1:{self.server.server_port}/tests/xaml-namespace-worker.js'}
...
before = [((24,24),RED),((104,24),WHITE),((64,24),WHITE)]
after = [((24,24),WHITE),((104,24),BLUE),((64,24),WHITE)]
def presented(page, expected):
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            raise AssertionError({'PresentedNamespacePixels': [(p, image.getpixel(p), c) for p,c in expected]})
        page.wait_for_timeout(25)
def evaluate(page, expression, arg=None):
    return page.evaluate('''async arg=>{let timer;try{return await Promise.race([
        Promise.resolve().then(()=>('''+expression+''')(arg)),new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error('Namespace fixture operation timeout')),45000);})
    ...
    with sync_playwright() as p:
        ...
        for mode in ['single', 'render-worker', 'full-isolation']:
            for aot in [False, True]:
                ...
                try:
                    page.goto(f'http://127.0.0.1:{server.server_port}/tests/namespace-entry.html?mode={mode}')
                    ...
                    state = evaluate(page, """async([mode,aot])=>{if(mode==='full-isolation')return await catalogHost.InvokeAsync('MountNamespaces',aot);
                        const A=await import('@wieslawsoltes/avalonia'),{CreateNamespaceScene}=await import('/tests/xaml-namespace-scene.js');
                        globalThis.namespaceScene=await CreateNamespaceScene(A,catalog.Root,aot);return namespaceScene.State();}""", [mode,aot])
                    assert state['Aot']==aot and state['HasDocument']==(mode!='full-isolation') and not state['RenderError'], state
                    assert state['Children']==2 and state['Left']==16 and state['Top']==16, state
                    assert state['Text']=='  Namespaces\n  preserved  ', state
                    initial, maximum = presented(page, before)
                    change = evaluate(page, "async mode=>mode==='full-isolation'?await catalogHost.InvokeAsync('ChangeNamespaces'):await namespaceScene.Change()", mode)
                    assert change['Left']==96, change
                    updated, error = presented(page, after); maximum=max(maximum,error)
                    current = evaluate(page, "async mode=>mode==='full-isolation'?await catalogHost.InvokeAsync('NamespaceState'):namespaceScene.State()", mode)
                    ...
                    else: evaluate(page,'()=>{namespaceScene.Dispose();catalog.Root.Dispose();}')
                    entry.update(Passed=True, MaximumChannelError=maximum, ComparedChannels=4, AutonomousRedraw=True,
                        IgnorableFiltered=True, QualifiedAttachedBindings=True, XmlWhitespacePreserved=True,
